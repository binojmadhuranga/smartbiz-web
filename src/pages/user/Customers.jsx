    ...
    <div className="p-2 xs:p-3 sm:p-6 lg:p-8 animate-fade-in">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-700/50">
            ...
            <tbody className="bg-gray-800/20 divide-y divide-gray-700/40">
              {customers.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-4 lg:px-6 py-8 lg:py-12 text-center text-gray-300 text-sm lg:text-base">
                ...
                customers.map((customer) => (
                  <tr key={customer.id} className="hover:bg-gray-700/30 transition-colors">
                    <td className="px-4 lg:px-6 py-4 lg:py-5 whitespace-nowrap">
                      <div className="text-sm lg:text-base font-medium text-gray-100">{customer.name}</div>
                      <div className="text-xs lg:text-sm text-gray-400">ID: {customer.id}</div>
                    </td>
                    <td className="px-4 lg:px-6 py-4 lg:py-5 whitespace-nowrap">
                      <div className="text-sm lg:text-base text-gray-200">{customer.email || 'N/A'}</div>
                      <div className="text-xs lg:text-sm text-gray-400">{customer.phone || 'N/A'}</div>
                    </td>
                    <td className="px-4 lg:px-6 py-4 lg:py-5 whitespace-nowrap">
                      <div className="text-sm lg:text-base text-gray-200">{customer.address || 'N/A'}</div>
                      <div className="text-xs lg:text-sm text-gray-400">{customer.city || ''} {customer.state || ''}</div>
                    </td>
                    <td className="px-4 lg:px-6 py-4 lg:py-5 whitespace-nowrap">
      ...
      <div className="md:hidden space-y-3 xs:space-y-4">
        {customers.length === 0 ? (
          ...
          customers.map((customer) => (
            <div key={customer.id} className="bg-gray-800/40 backdrop-blur-md rounded-lg xs:rounded-xl shadow-lg border border-gray-700/50 overflow-hidden">
              {/* Customer Header */}
              <div className="p-3 xs:p-4 border-b border-gray-700/50">
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0 pr-2">
                    <h3 className="font-semibold text-gray-100 text-base xs:text-lg truncate">{customer.name}</h3>
                    <p className="text-xs xs:text-sm text-gray-400 mt-1">ID: {customer.id}</p>
                  </div>
                  <div className="flex space-x-1 xs:space-x-2 flex-shrink-0">
                    <Link
                      to={`/dashboard/customers/${customer.id}/edit`}
                      className="p-1.5 xs:p-2 text-green-400 hover:bg-gray-700/50 rounded-md xs:rounded-lg transition-colors"
                    >
                    ...
                    <button
                      onClick={() => handleDelete(customer.id)}
                      className="p-1.5 xs:p-2 text-red-400 hover:bg-gray-700/50 rounded-md xs:rounded-lg transition-colors"
                    >
                      <svg className="w-3.5 h-3.5 xs:w-4 xs:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="p-2 xs:p-3 sm:p-4 space-y-2 xs:space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-1.5 xs:space-x-2">
                    ...
                    <span className="text-xs xs:text-sm text-gray-300">Status:</span>
                  </div>
                ...
                <div className="space-y-1.5 xs:space-y-2">
                  <div className="flex items-center space-x-1.5 xs:space-x-2">
                    ...
                    <span className="text-xs xs:text-sm text-gray-300">Email:</span>
                    <span className="text-xs xs:text-sm text-gray-100 truncate">{customer.email || 'No email'}</span>
                  </div>
                  <div className="flex items-center space-x-1.5 xs:space-x-2">
                    <svg className="w-3 xs:w-4 h-3 xs:h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                    </svg>
                    <span className="text-xs xs:text-sm text-gray-300">Phone:</span>
                    <span className="text-xs xs:text-sm text-gray-100 truncate">{customer.phone || 'No phone'}</span>
                  </div>
