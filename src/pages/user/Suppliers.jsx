  ...
  return (
    <div className="p-2 xs:p-3 sm:p-4 md:p-4 lg:p-6 xl:p-8 max-w-full overflow-hidden">
      ...
      <div className="mb-4 md:mb-6">
        ...
        {/* Search Bar */}
        <div className="bg-gray-800/40 backdrop-blur-md rounded-lg shadow-lg border border-gray-700/50 p-3 md:p-4">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-700/50">
            <thead className="bg-gray-900/60">
              <tr>
                <th className="px-2 md:px-3 lg:px-4 xl:px-6 py-2 md:py-3 text-left text-xs font-medium text-gray-200 uppercase tracking-wider">
                  Supplier Info
                </th>
                <th className="px-2 md:px-3 lg:px-4 xl:px-6 py-2 md:py-3 text-left text-xs font-medium text-gray-200 uppercase tracking-wider">
                  Contact
          ...
          <tbody className="bg-gray-800/20 divide-y divide-gray-700/40">
            {suppliers.length === 0 ? (
              <tr>
                <td colSpan="5" className="px-2 md:px-3 lg:px-4 xl:px-6 py-6 md:py-8 text-center text-gray-300 text-sm md:text-base">
                  {search ? 'No suppliers found matching your search.' : 'No suppliers found. Create your first supplier!'}
                </td>
              </tr>
            ) : (
              suppliers.map((supplier, index) => (
                <tr key={supplierKey(supplier, index)} className="hover:bg-gray-700/30 transition-colors">
                  <td className="px-2 md:px-3 lg:px-4 xl:px-6 py-2 md:py-3 lg:py-4 whitespace-nowrap">
                    <div className="text-sm md:text-base font-medium text-gray-100 truncate">{supplier.name}</div>
                    <div className="text-xs md:text-sm text-gray-400 truncate">ID: {supplier.id}</div>
                  </td>
                  <td className="px-2 md:px-3 lg:px-4 xl:px-6 py-2 md:py-3 lg:py-4 whitespace-nowrap">
                    <div className="text-sm md:text-base text-gray-200 truncate">{supplier.email || 'N/A'}</div>
                    <div className="text-xs md:text-sm text-gray-400 truncate">{supplier.phone || 'N/A'}</div>
                  </td>
                  <td className="px-2 md:px-3 lg:px-4 xl:px-6 py-2 md:py-3 lg:py-4 whitespace-nowrap">
                    <div className="text-sm md:text-base text-gray-200 max-w-[120px] md:max-w-xs lg:max-w-sm truncate">{supplier.address || 'N/A'}</div>
                  </td>
                  <td className="px-2 md:px-3 lg:px-4 xl:px-6 py-2 md:py-3 lg:py-4 whitespace-nowrap">
                    <div className="text-sm md:text-base text-gray-200 max-w-[100px] md:max-w-xs truncate">
                      {(() => {
                        const supplierId = getSupplierId(supplier);
                  ...
                  <td className="px-2 md:px-3 lg:px-4 xl:px-6 py-2 md:py-3 lg:py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center space-x-1 md:space-x-2">
                      <button
                        ...
                        className="text-green-400 hover:text-green-300 transition-colors text-xs md:text-sm"
                      >
                        Edit
                      </button>
                      <button
                        ...
                        className="text-red-400 hover:text-red-300 transition-colors disabled:opacity-50 text-xs md:text-sm"
                      >
      ...
      <div className="md:hidden space-y-4">
        {suppliers.length === 0 ? (
          ...
          suppliers.map((supplier, index) => (
            <div key={supplierKey(supplier, index)} className="bg-gray-800/40 backdrop-blur-md rounded-lg shadow-lg border border-gray-700/50 overflow-hidden">
              {/* Supplier Header */}
              <div className="p-4 border-b border-gray-700/50">
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0 pr-2">
                    <h3 className="font-semibold text-gray-100 text-lg truncate">{supplier.name}</h3>
                    <p className="text-sm text-gray-400 mt-1">ID: {supplier.id}</p>
                  </div>
                  <div className="flex space-x-2 flex-shrink-0">
                    <button
                      onClick={() => handleEdit(supplier)}
                      className="p-2 text-green-400 hover:bg-gray-700/50 rounded-lg transition-colors"
                    >
                    ...
                    <button
                      onClick={() => handleDelete(supplier.id, supplier.name)}
                      disabled={deleteLoading === supplier.id}
                      className="p-2 text-red-400 hover:bg-gray-700/50 rounded-lg transition-colors disabled:opacity-50"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                      </svg>
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-100 truncate">{supplier.email || 'No email'}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className="text-sm text-gray-100 truncate">{supplier.phone || 'No phone'}</span>
                  </div>
